...
function Deploy
{
    ...
    function CreateVideoIndexerFunctionApp
    {
        ...
        # $publishingProfile = Get-AzWebAppPublishingProfile -ResourceGroupName $resourceGroupName -Name $videoIndexerFunctionAppname
        # $functionCode = Invoke-RestMethod `
        #     -Uri "https://$videoIndexerFunctionAppname.scm.azurewebsites.net/api/functions/admin/token" `
        #     -Headers @{ Authorization = "Basic $($publishingProfile)" -f $base64Credentials) }
